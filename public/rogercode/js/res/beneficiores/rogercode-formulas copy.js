/* asigna varios elementos HTML a variables y luego establece un "escucha" de evento de cambio en un elemento con el ID "plantasacrificio_id".
Cuando el valor de este elemento cambia, se llama a la función "obtener_registroid" con el nuevo valor como argumento. */

/* 1. La primera línea asigna el elemento HTML con el ID "plantasacrificio_id" a una variable llamada "plantasacrificio_id" utilizando la función "$" de jQuery.  */
...
/* 5. La función "plantasacrificio_id.change" establece un "escucha" de evento de cambio en el elemento HTML con el ID "plantasacrificio_id". Cuando el valor de este elemento cambia, se llama a la función dentro de las llaves.  */
/* 
6. La primera línea dentro de la función asigna el elemento HTML con el ID "plantasacrificio_id" a una variable con el mismo nombre.  
7. La segunda línea llama a una función llamada "obtener_registroid" con el valor de la variable "plantasacrificio_id" como argumento.

En resumen, el código asigna varios elementos HTML a variables y establece un "escucha" de evento de cambio en un elemento con el ID "plantasacrificio_id" para llamar a una función cuando su valor cambia. 
...
/**********************************************************/
valorMacho.change(function () {
    CalculateTotalMacho();
});
cantidadMacho.change(function () {
    CalculateTotalMacho();
});
valorHembra.change(function () {
    CalculateTotalHembra();
});
cantidadHembra.change(function () {
    CalculateTotalHembra();
});
...
function calculatotales() {
    ...
    //COSTO ANIMAL 1 / 2 / 3
    var costopie1 = formatMoneyNumber($("#costoanimal1").val());
    $("#costopie1").val(formatCantidadSinCero(pesopie1 * costopie1));
    var tpie1 = Number(pesopie1 * costopie1);
    var costopie2 = formatMoneyNumber($("#costoanimal2").val());
    $("#costopie2").val(formatCantidadSinCero(pesopie2 * costopie2));
    var tpie2 = Number(pesopie2 * costopie2);
    var costopie3 = formatMoneyNumber($("#costoanimal3").val());
    $("#costopie3").val(formatCantidadSinCero(pesopie3 * costopie3));
    var tpie3 = Number(pesopie3 * costopie3);

    //SACRIFICIO / FOMENTO / DEGUELLO / BASCULA / TRANSPORTE
    var sacrificio = formatMoneyNumber($("#sacrificio").val());
    $("#tsacrificio").val(formatCantidadSinCero(cantidad * sacrificio));
    var tsacrif = Number(cantidad * sacrificio);
    var fomento = formatMoneyNumber($("#fomento").val());
    $("#tfomento").val(formatCantidadSinCero(cantidad * fomento * -1));
    var tfomen = Number(cantidad * fomento * -1);
    var deguello = formatMoneyNumber($("#deguello").val());
    $("#tdeguello").val(formatCantidadSinCero(cantidad * deguello));
    var tdgue = Number(cantidad * deguello);
    var bascula = formatMoneyNumber($("#bascula").val());
    $("#tbascula").val(formatCantidadSinCero(cantidad * bascula * -1));
    var tbascu = Number(cantidad * bascula * -1);
    var transporte = formatMoneyNumber($("#transporte").val());
    $("#ttransporte").val(formatCantidadSinCero(cantidad * transporte));
    var ttrans = Number(cantidad * transporte);

    //TOTAL PIELES Y VISCERAS
    var pielescosto = formatMoneyNumber($("#pielescosto").val());
    $("#tpieles").val(formatCantidadSinCero(pieleskg * pielescosto * -1));
    var tpielc = Number(pieleskg * pielescosto * -1);
    ...
    var visceras = formatMoneyNumber($("#visceras").val());
    $("#tvisceras").val(formatCantidadSinCero(visceras * -1));
    var tvisce = Number(visceras * -1);
    ...
    $("#valorfactura").val(
        formatCantidadSinCero(tpie1 + tpie2 + tpie3 + tfomen + tbascu)
    );
    if (canalcaliente != "") {
        //$('#costokilo').val(formatCantidadSinCero(Math.round(totalc / canalplanta)));//valiating
        $("#costokilo").val(
            formatCantidadSinCero(
                Math.round(totalCantidadCostos / canalcaliente)
            )
        ); //valiating
        $("#costo").val(
            formatCantidadSinCero(
                Math.round(totalCantidadCostos / canalcaliente) * 12.5
            )
        ); //validating
    } else {
        $("#costokilo").val(0);
    ...
    if (canalcaliente != "" && pesopierend != 0) {
        ...
        $("#rendcaliente").val(
            formatCantidad(((canalcaliente / pesopierend) * 100).toFixed(2))
        );
    } else {
        $("#rendcaliente").val(0);
    }
    if (canalplanta != "" && pesopierend != 0) {
        ...
        $("#rendplanta").val(
            formatCantidad(((canalplanta / pesopierend) * 100).toFixed(2))
        );
    } else {
        $("#rendplanta").val(0);
    }
    ...
    if (canalfria != "" && pesopierend != 0) {
        $("#rendfrio").val(
            formatCantidadSinCero(((canalfria / pesopierend) * 100).toFixed(2))
        );
    } else {
        $("#rendfrio").val(0);
    }
...
const CalculateTotalMacho = () => {
    ...
    let valorTotal = $("#valorTotalMacho").val(
        formatCantidadSinCero(cantidadMacho * valorUnitarioMacho)
    );
...
const CalculateTotalHembra = () => {
    ...
    let valorUnitarioHembra = formatMoneyNumber(
        $("#valorUnitarioHembra").val()
    );
    let valorTotal = $("#valorTotalHembra").val(
        formatCantidadSinCero(cantidadHembra * valorUnitarioHembra)
    );
